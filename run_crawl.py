...
if __name__ == "__main__":
    ...
    scraper = MoviesScraper(release_date_from='2024-01-01', release_date_to='2024-01-10')
    movie_data = scraper.fetch_movies()

    file_path = 'movies_reviews.json'
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        new_movies_reviews = list(executor.map(lambda movie: fetch_movie_reviews(movie.get('Movie ID'), movie.get('Title')), movie_data[:3]))
